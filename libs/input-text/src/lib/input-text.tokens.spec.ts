...
describe('Input Tokens', () => {
  it('should export HA_INPUT_TEXT_TOKENS with CSS variable name strings', () => {
    ...
    expect(HA_INPUT_TEXT_TOKENS.bg).toBe('--ha-input-bg');
    expect(HA_INPUT_TEXT_TOKENS.color).toBe('--ha-input-color');
    expect(HA_INPUT_TEXT_TOKENS.border).toBe('--ha-input-border');
    expect(HA_INPUT_TEXT_TOKENS.radiusSm).toBe('--ha-input-radius-sm');
    expect(HA_INPUT_TEXT_TOKENS.radiusMd).toBe('--ha-input-radius-md');
    expect(HA_INPUT_TEXT_TOKENS.radiusLg).toBe('--ha-input-radius-lg');
  });

  it('should include all required token keys from the design spec', () => {
    const keys = Object.keys(HA_INPUT_TEXT_TOKENS);
    const required = [
      'bg',
      'color',
      'border',
  ...
  it('should have all values prefixed with --ha-input-', () => {
    const values = Object.values(HA_INPUT_TEXT_TOKENS);
  ...
  it('every HA_INPUT_TEXT_TOKENS value MUST be a key of HA_COMPONENT_TOKEN_DEFAULTS (foundation provides a default for every input token)', () => {
    ...
    for (const cssVarName of Object.values(HA_INPUT_TEXT_TOKENS)) {
