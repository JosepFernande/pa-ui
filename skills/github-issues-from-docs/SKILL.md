...
description:
  ...
  técnica usando cache híbrido de GitHub Wiki en Engram.'
license: Apache-2.0
metadata:
  author: jfernandezo
  version: '3.0'
...
2. **Cache híbrido obligatorio** sobre la documentación de referencia del Wiki
   ...
   (`wiki-docs/index`) + contenido completo por página (`wiki-docs/{page-slug}`)
   con `last_synced_head_sha` / `last_commit_sha`.
...
5. **Auto-bootstrap**: si el índice no existe en Engram, ejecutar
   `/sync-wiki-docs` automáticamente antes de continuar.

## Decision Gates

| Situación                             | Acción                                                     |
| ------------------------------------- | ---------------------------------------------------------- |
| Índice de docs no existe en Engram    | Ejecutar `/sync-wiki-docs`                                 |
...
| HEAD actual == `last_synced_head_sha` | Todo fresco, usar cache de Engram (0 lecturas de archivo)  |
...
2. Determinar el tema del issue (keywords) y cargar el índice de docs desde
   Engram (local, 0 operaciones git)
...
2. Cargar y validar páginas relevantes (mismo flujo híbrido)
