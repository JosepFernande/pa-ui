...
description:
  'Trigger: pa-ui, Angular UI library, pa- components, design tokens, theming,
  sdd design spec tasks apply verify. Apply the six hard rules and three-layer
  token system to every pa-ui artifact.'
license: MIT
metadata:
...
6. **Consistent APIs.** Naming, inputs, outputs, and variants follow project
   conventions: `pa-` prefix, `size=sm|md|lg`, `variant=solid|outline|ghost`,
...
3. **Component** — `--pa-button-bg`, `--pa-button-color`,
   `--pa-input-focus-ring`.
...
off-limits inside components. The Theme Engine (`providePaTheme()`) auto-derives
