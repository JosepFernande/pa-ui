...
export const rule = createRule({
  meta: {
    ...
    docs: {
      description:
        'Disallow union literal types on @Input() color properties. Color must be typed as string.',
    },
    messages: {
      colorNotString:
        '@Input() color must be typed as `string`, not a union of string literals.',
    },
    schema: [],
  },
  ...
  create(context) {
     ...
     */
    function checkDecoratorInput(node: TSESTree.PropertyDefinition): void {
      if (node.key.type !== AST_NODE_TYPES.Identifier) return;
      if (node.key.name !== 'color') return;
     ...
     */
    function checkSignalInput(node: TSESTree.PropertyDefinition): void {
      if (node.key.type !== AST_NODE_TYPES.Identifier) return;
      if (node.key.name !== 'color') return;
