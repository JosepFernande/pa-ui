...
function isHardcodedColor(value: string): boolean {
  ...
  // Allowed keywords
  if (ALLOWED_KEYWORDS.has(trimmed)) return false;

  // Hex colors: #rgb, #rrggbb, #rgba, #rrggbbaa
  if (/^#[0-9a-f]{3,8}$/i.test(trimmed)) return true;

  // rgb/rgba/hsl/hsla functions
  if (/^(rgb|rgba|hsl|hsla)\(/i.test(trimmed)) return true;
...
/**
 * PostCSS plugin function for stylelint rule `halo-ui/no-hardcoded-colors`.
 *
...
export function ruleFunction(root: Root, result: Result): void {
  root.walkDecls((node: Declaration) => {
    ...
    const isColorProp =
      /^(color|background(-color)?|border(-(top|right|bottom|left))?-color|outline(-color)?|caret-color|text-decoration-color|accent-color|fill|stroke|text-emphasis-color|column-rule-color|scrollbar-color)$/.test(
        prop,
      );
    ...
    if (!isColorProp) {
      ...
      const isShorthand =
        /^(border(-(top|right|bottom|left))?|outline|background|box-shadow|text-shadow)$/.test(
          prop,
        );
    ...
    for (const part of parts) {
      ...
      if (isHardcodedColor(cleaned)) {
        result.warn(
          `Hardcoded color "${cleaned}" is not allowed. Use a CSS custom property (token) instead. (halo-ui/no-hardcoded-colors)`,
          { node, word: cleaned },
        );
