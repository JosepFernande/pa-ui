...
describe('no-hardcoded-colors', () => {
  ...
  it('should allow var(--*) token references', async () => {
    const result = await run('.ha-button { color: var(--ha-text); }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow var(--*) with fallback', async () => {
    const result = await run('.ha-button { color: var(--ha-text, #333); }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow transparent keyword', async () => {
    const result = await run('.ha-button { background: transparent; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow currentColor keyword', async () => {
    const result = await run('.ha-icon { color: currentColor; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow inherit keyword', async () => {
    const result = await run('.ha-button { color: inherit; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow initial keyword', async () => {
    const result = await run('.ha-button { color: initial; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow unset keyword', async () => {
    const result = await run('.ha-button { color: unset; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow non-color properties with hardcoded values', async () => {
    const result = await run('.ha-button { display: flex; opacity: 0.5; z-index: 10; }');
    expect(result.messages).toHaveLength(0);
  });

  it('should allow border shorthand with var() for color', async () => {
    const result = await run('.ha-button { border: var(--ha-button-border); }');
    expect(result.messages).toHaveLength(0);
  });
  ...
  it('should reject hex colors (3-digit)', async () => {
    const result = await run('.ha-button { color: #333; }');
