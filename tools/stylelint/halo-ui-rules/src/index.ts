...
function makeRule(ruleName: string, fn: (root: any, result: any, secondary?: any) => void) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (primaryOption: boolean, secondaryOption?: any) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export default [
  ...
  // @ts-expect-error -- stylelint v17 type expects Rule shape but createPlugin accepts any function
  createPlugin(
    'halo-ui/no-hardcoded-colors',
    makeRule('halo-ui/no-hardcoded-colors', noHardcodedColors),
  ),
  // @ts-expect-error -- stylelint v17 type expects Rule shape but createPlugin accepts any function
  createPlugin(
    'halo-ui/no-hardcoded-spacing-radius',
    makeRule('halo-ui/no-hardcoded-spacing-radius', noHardcodedSpacingRadius),
  ),
  // @ts-expect-error -- stylelint v17 type expects Rule shape but createPlugin accepts any function
  createPlugin(
    'halo-ui/no-ng-deep-host-context',
    makeRule('halo-ui/no-ng-deep-host-context', noNgDeepHostContext),
  ),
  // @ts-expect-error -- stylelint v17 type expects Rule shape but createPlugin accepts any function
  createPlugin(
    'halo-ui/no-important-outside-host',
    makeRule('halo-ui/no-important-outside-host', noImportantOutsideHost),
  ),
];
