...
/**
 * PostCSS plugin function for stylelint rule `halo-ui/no-important-outside-host`.
 *
...
export function ruleFunction(root, result) {
    root.walkDecls((node) => {
        ...
        if (!isInsideHostScope(node)) {
            result.warn(`"!important" is only allowed inside ":host" scope. Move this declaration inside ":host { ... }". (halo-ui/no-important-outside-host)`, { node, word: '!important' });
        }
    });
}
