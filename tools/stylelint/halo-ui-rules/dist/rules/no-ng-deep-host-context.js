...
export function ruleFunction(root, result) {
    root.walkRules((node) => {
        ...
        if (selector.includes('::ng-deep')) {
            result.warn(`"::ng-deep" is banned. Use component CSS custom properties (tokens) for style overrides. (halo-ui/no-ng-deep-host-context)`, { node, word: '::ng-deep' });
        ...
        if (selector.includes(':host-context')) {
            ...
            if (hasNonTokenDecl) {
                result.warn(`":host-context()" is only allowed for CSS custom property (token) overrides. Found non-"----" declarations. (halo-ui/no-ng-deep-host-context)`, { node, word: ':host-context' });
            }
        }
    });
}
