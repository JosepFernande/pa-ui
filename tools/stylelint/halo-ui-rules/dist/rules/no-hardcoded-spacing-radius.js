...
/**
 * PostCSS plugin function for stylelint rule `halo-ui/no-hardcoded-spacing-radius`.
 *
...
export function ruleFunction(root, result) {
    root.walkDecls((node) => {
        ...
        if (!SPACING_RADIUS_PROPS.has(prop) &&
        ...
        for (const part of parts) {
            ...
            if (/^-?\d+\.?\d*(px|rem|em)$/.test(trimmed)) {
                result.warn(`Hardcoded spacing/radius "${trimmed}" is not allowed. Use a CSS custom property (token) instead. (halo-ui/no-hardcoded-spacing-radius)`, { node, word: trimmed });
